...
from market_intel.scoring.confidence import EXPECTED_FACT_FIELDS, ConfidenceInputs, compute_confidence, extraction_completeness
...
from market_intel.scoring.surprise import composite_surprise, guidance_direction_score, raw_surprise, zscore
...
def run_pipeline(
    ...
    with connect(db_path) as conn:
        raw_items = _ingest(tickers, lookback_days)
        ...
        for event in events:
            _enrich_facts(conn, event, earnings_provider)
            event.exposure = get_exposure_for_ticker(event.ticker, conn)
            _populate_market_data(conn, event, market_data, options_provider, price_cache)
...
def _ingest(tickers: list[str], lookback_days: int) -> list:
    ...
    logger.info(
        "Ingested %d raw items (SEC live, news %s)", len(items), "MOCKED" if news.is_mocked else "live"
    )
    return items
...
def _enrich_facts(conn, event: Event, earnings_provider) -> None:
    if event.event_type != EventType.EARNINGS:
        return

    event_dt = parse_utc(event.timestamp_utc)
    period = current_fiscal_period(event_dt)
...
def _populate_market_data(conn, event: Event, market_data: MarketDataIngestor, options_provider, cache) -> None:
...
def _score_event(conn, event: Event) -> None:
    ...
    if event.event_type == EventType.EARNINGS:
        ...
        eps_raw = raw_surprise(event.facts.eps_actual, event.facts.eps_consensus)
        rev_raw = raw_surprise(event.facts.revenue_actual, event.facts.revenue_consensus)
        ...
        eps_z = zscore(eps_raw, eps_history)
        rev_z = zscore(rev_raw, rev_history)
        guidance_score = guidance_direction_score(event.facts.guidance_direction)
        composite = composite_surprise(eps_z, rev_z, guidance_score)
        # Not enough trailing history to z-score yet -> fall back to raw surprise
        # (still informative, just not cross-company comparable).
        if composite is None:
            composite = composite_surprise(eps_raw, rev_raw, guidance_score)
    else:
        composite = guidance_direction_score(event.facts.guidance_direction)

    interp.surprise_score = composite
    if composite is not None:
        interp.forward_earnings_effect = "positive" if composite > 0.05 else "negative" if composite < -0.05 else "neutral"
    ...
    confidence_inputs = ConfidenceInputs(
        ...
        already_priced_in=0.0,  # MVP: no pre-event drift signal available; see README
    ...
    interp.confirmation_score = confirmation.score

    # --- Verification flag -------------------------------------------
    impact = max(abs(composite) if composite is not None else 0.0, confirmation.score)
    from market_intel.scoring.confidence import needs_verification
